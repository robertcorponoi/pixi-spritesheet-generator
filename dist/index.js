...
program
    ...
    .option("-o, --output <path>", "The path to the directory where the spritesheet and data file will be saved.", process.cwd())
    .option("-t, --trim", "Indicates whether transparent whitespace around the sprites should be trimmed or not.", false)
    .option("-c, --columns <number>", "The number of columns in the spritesheet. If not provided, the spritesheet will be a single column.", "1")
    ...
    const spritePlacement = input.map((inputFile) => {
        ...
        if (spriteNameSplit.length) {
            const spriteLastElement = spriteNameSplit[spriteNameSplit.length - 1];
            if (spriteLastElement) {
                // We consider the sprite an animation if it has a number
                // as the last element in its name.
                const spriteIsAnimation = spriteLastElement.match(/^[0-9]+$/);
                if (spriteIsAnimation) {
                    ...
                    const spriteAnimationName = spriteNameSplit
                        .slice(0, -1)
                        .join("-");
                    const spriteHasAnimationEntry = animations[spriteAnimationName];
    ...
    if (options.trim) {
        ...
        shell.exec(`magick ${inputFiles} -trim +repage -set filename:base "%[basename]" "${path.join(trimmedSpritesDir, "%[filename:base].png")}"`);
    }
    ...
    shell.exec(`magick montage ${options.trim ? `"${trimmedSpritesDir}/*.*"` : inputFiles} -mode Concatenate${tiles !== null ? ` -tile ${tiles}` : ""} -background transparent "${spritesheetOutput}"`);
    ...
    await fsPromises.writeFile(path.join(options.output, `${name}.json`), JSON.stringify(jsonData, null, 4), "utf-8");
    if (options.trim)
        await rimraf(trimmedSpritesDir);
    spinner.stop();
})
    .parse();
